...
<body class="bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
    ...
    <div class="text-center mb-12">
        <h1 class="text-4xl md:text-5xl font-bold text-slate-800 mb-4 leading-tight">
            ...
            <span class="text-transparent bg-clip-text bg-gradient-to-r from-teal-600 to-cyan-600">
                Buscador de Familias por planes de cuidado
            </span>
        </h1>
        <p class="text-slate-600 text-lg max-w-2xl mx-auto leading-relaxed">
            Aquí puedes ver las familias registradas sin planes de cuidado asignados. Utiliza los filtros para encontrar familias específicas según microterritorio o responsable.
        </p>
    </div>
    ...
    <main class="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2">
            ...
            <div class="col-span-2 md:col-span-1 text-md font-semibold my-4 sm:mr-4">
                <div class="flex items-center mb-4">
                    <label for="familiograma" class="font-semibold">Estado</label>
                </div>
                <select id="filtroEstado"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                    <option value="">Todos</option>
                    <?php foreach ($estados as $estado): ?>
                        <option value="<?php echo h($estado); ?>">
                            <?php echo h($estado); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <table id="miTabla" style="width:100%;" class="stripe hover text-sm text-left text-gray-600 border border-gray-200 rounded-lg overflow-hidden">
                <thead class="bg-gray-200 font-medium border-b border-gray-300">
                    <tr class=" text-gray-900 font-light">
                        ...
                        <th class="px-4 py-2 font-semibold text-center cursor-pointer hover:bg-gray-100">ID Familia</th>
                        <th class="px-4 py-2 font-semibold text-center cursor-pointer hover:bg-gray-100">ID Observacion</th>
                        <th class="px-4 py-2 font-semibold text-center cursor-pointer hover:bg-gray-100">Fecha</th>
                        <th class="px-4 py-2 font-semibold text-center cursor-pointer hover:bg-green-100">Estado</th>
                        ...
                        <th class="px-2 py-2 font-semibold text-center cursor-pointer hover:bg-green-100">Plan de Cuidado</th>
                        <th class="px-2 py-2 font-semibold text-center cursor-pointer hover:bg-green-100">Familiograma</th>
    ...
    <script>
        ...
        const URL_view = "<?php echo $this->Html->url(['controller' => 'Familias','action' => 'view', '__ID__']); ?>";
        <?php if (!isset($tipoUsuario)) {
            $tipoUsuario = null;
        } ?>
        const URL_edit = "<?php
                            echo $this->Html->url([ 'controller' => 'Observacions','action' => 'add_plancuidado', '__ID__']);
                            ?>";
        ...
        $(document).ready(function() {
            ...
            const table = $miTabla.DataTable({
                ...
                responsive: {
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-4 mt-2 w-full max-w-md mx-auto">
                        ...
                        <div class="flex gap-2 mt-4">
                            ...
                            <a href="${URL_edit.replace('__ID__', data.id_observacion)}" class="bg-gray-200 hover:bg-amber-600 text-teal-700 px-3 py-1 rounded text-sm">Actualizar</a>
                        </div>
                    </div>
                `;
                        }
                ...
                serverSide: true,
                ajax: {
                    url: "<?php echo $this->Html->url(['controller' => 'Observacions', 'action' => 'plancuidadoIndex']); ?>",
                    type: "GET",
                    data: function(d) {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            const planUrl = "<?php echo $this->Html->url(['controller' => 'Familias', 'action' => 'plancuidado', '__ID__']); ?>";
                            return `<a href="${planUrl.replace('__ID__', row.id_familia)}" class="text-teal-600 hover:text-teal-800 underline font-medium">Ver Plan</a>`;
                        }
                    },
                    {
                        data: "familiograma",
                    ...
                    },
                    {
                        ...
                        searchable: false,
                        render: function(data, type, row) {
                            const viewUrl = URL_view.replace('__ID__', row.id_familia);
                            const editUrl = URL_edit.replace('__ID__', row.id_observacion);
                            return `
